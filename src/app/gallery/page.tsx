...
export default function GalleryPage() {
  ...
  const filtered = gallery
    ...
    .filter((img) => !searchQuery || img.alt.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <>
      <PageHeader
        ...
        image="/images/demo/elephants-sunset.jpg"
      />
      ...
      <Section>
        ...
        {/* Toolbar: search, filter chips, column toggle */}
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-20 text-center">
            ...
            <p className="text-ink-soft">No images match your search.</p>
            <button
              type="button"
              ...
              <Reveal key={img.src} delay={Math.min(i * 60, 400)}>
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-black/70 via-black/5 to-transparent p-4 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    <span className="text-sm font-medium text-white drop-shadow-sm">{img.alt}</span>
                    <span className="mt-0.5 text-xs text-white/70">{img.category}</span>
                  </div>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-3 text-sm text-ink-soft">
          <p>
            ...
            {activeCategory !== "All" && !searchQuery && <> in <span className="font-medium text-foreground">{activeCategory}</span></>}
          </p>
        ...
        >
          ...
          <div className="absolute left-0 right-0 top-0 z-10 flex items-center justify-between px-4 py-4 sm:px-6">
                ...
                {filtered.map((_, i) => (
                  <div
                    ...
                    className={cn(
                      ...
                      i === lightboxIdx ? "w-6 bg-gold" : "w-1 bg-white/20",
                    )}
                  />
                ))}
              </div>
            ...
            <div className="flex items-center gap-2">
              {filtered.length > 1 && (
                <button
                  ...
                  className={cn(
                    ...
                    slideshow ? "bg-gold text-neutral-900" : "bg-white/10 text-white hover:bg-white/20",
                  )}
              ...
              <button
                type="button"
                onClick={() => { setLightbox(false); setSlideshow(false); }}
                className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20 transition-colors"
          ...
          {filtered.length > 1 && (
            <>
              <button
                type="button"
                onClick={goPrev}
                className="absolute left-2 top-1/2 z-10 flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full bg-white/10 text-white opacity-0 hover:bg-white/20 hover:opacity-100 focus:opacity-100 transition-all sm:left-4 lg:opacity-0 lg:group-hover:opacity-100"
              ...
              <button
                type="button"
                onClick={goNext}
                className="absolute right-2 top-1/2 z-10 flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full bg-white/10 text-white opacity-0 hover:bg-white/20 hover:opacity-100 focus:opacity-100 transition-all sm:right-4 lg:opacity-0 lg:group-hover:opacity-100"
          ...
          {/* Image */}
          <div
            className="group relative flex max-h-[80vh] max-w-[95vw] flex-col items-center"
          ...
          >
            ...
            <div className="mt-4 text-center">
              <p className="text-base font-medium text-white">{filtered[lightboxIdx].alt}</p>
              <p className="mt-0.5 text-sm text-white/50">{filtered[lightboxIdx].category}</p>
            </div>
          </div>

          {/* Bottom progress bar for slideshow */}
          {slideshow && (
